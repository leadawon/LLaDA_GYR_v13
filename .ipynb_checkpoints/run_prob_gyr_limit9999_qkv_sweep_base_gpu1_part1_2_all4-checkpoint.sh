...
SCRIPT_LOG_DIR="${SCRIPT_LOG_DIR:-out/logs/prob_gyr_limit9999_qkv_sweep_llada_gpu${GPU_ID}_part1_2}"
mkdir -p "${SCRIPT_LOG_DIR}" "${ROOT_OUT_HUMANEVAL}" "${LOG_DIR_HUMANEVAL}"
RUN_TAG="prob_gyr_limit9999_qkv_sweep_llada_gpu${GPU_ID}_part1_2_s${SHARD_INDEX}of${SHARD_COUNT}_$(date +%Y%m%d_%H%M%S)"
...
export LM_EVAL_INCLUDE_PATH="${LM_EVAL_INCLUDE_PATH:-/workspace/LLaDA_GYR_v13/data/tasks}"
...
echo "[start-part1_2-limit9999] gpu=${GPU_ID} (humaneval_instruct only, limit=${LIMIT_HUMANEVAL})"
...
for raw_tier in "${TIER_LIST[@]}"; do
  ...
  echo "============================================================"
  run_task_tier "humaneval_instruct" 512 "${LIMIT_HUMANEVAL}" "${ROOT_OUT_HUMANEVAL}" "${LOG_DIR_HUMANEVAL}" "${tier}"
done

echo "[done-part1_2-limit9999] gpu=${GPU_ID} (humaneval_instruct only)"
