...
SCRIPT_LOG_DIR="${SCRIPT_LOG_DIR:-out/logs/prob_gyr_limit9999_qkv_sweep_llada_gpu${GPU_ID}_part2}"
mkdir -p "${SCRIPT_LOG_DIR}" "${ROOT_OUT_MBPP}" "${ROOT_OUT_IFEVAL}" "${LOG_DIR_MBPP}" "${LOG_DIR_IFEVAL}"
RUN_TAG="prob_gyr_limit9999_qkv_sweep_llada_gpu${GPU_ID}_part2_s${SHARD_INDEX}of${SHARD_COUNT}_$(date +%Y%m%d_%H%M%S)"
...
for raw_tier in "${TIER_LIST[@]}"; do
  ...
  echo "[tier] ${tier} -> mbpp"
  echo "============================================================"
  run_task_tier "mbpp_instruct" 768 "${LIMIT_MBPP}" "${ROOT_OUT_MBPP}" "${LOG_DIR_MBPP}" "${tier}"

  echo "============================================================"
  echo "[tier] ${tier} -> ifeval"
  echo "============================================================"
  run_task_tier "ifeval" 768 "${LIMIT_IFEVAL}" "${ROOT_OUT_IFEVAL}" "${LOG_DIR_IFEVAL}" "${tier}"
done
