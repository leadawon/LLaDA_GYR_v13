...
SCRIPT_LOG_DIR="${SCRIPT_LOG_DIR:-out/logs/prob_gyr_limit9999_qkv_sweep_llada_gpu${GPU_ID}_part1}"
mkdir -p "${SCRIPT_LOG_DIR}" "${ROOT_OUT_GSM8K}" "${ROOT_OUT_HUMANEVAL}" "${LOG_DIR_GSM8K}" "${LOG_DIR_HUMANEVAL}"
RUN_TAG="prob_gyr_limit9999_qkv_sweep_llada_gpu${GPU_ID}_part1_s${SHARD_INDEX}of${SHARD_COUNT}_$(date +%Y%m%d_%H%M%S)"
